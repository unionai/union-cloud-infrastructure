variable "role_name" {
  ...
  type        = string
  default     = "unionai-provisioner-role"
}
...
resource "aws_iam_managed_policy" "admin_policy" {
  name        = "unionai-admin-policy-for-provisioner-role"
  policy = file("../admin.json")
}

resource "aws_iam_managed_policy" "manager_policy" {
  name        = "unionai-manager-policy-for-provisioner-role"
  policy = file("../manager.json")
}

resource "aws_iam_managed_policy" "provisioner_policy" {
  name        = "unionai-provisioner-policy-for-provisioner-role"
  policy = file("../provisioner.json")
}

resource "aws_iam_managed_policy" "reader_policy" {
  name        = "unionai-reader-policy-for-provisioner-role"
...
resource "aws_iam_role_policy_attachment" "provisioner_provisioner_policy_attachment" {
  role       = aws_iam_role.provisioner_role.name
  policy_arn = aws_iam_managed_policy.provisioner_policy.arn
}
